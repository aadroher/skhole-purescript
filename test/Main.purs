...
import Test.Spec (it, itOnly, describe, sequential)
import Test.Spec.Assertions (shouldEqual)
import Test.Spec.Reporter (consoleReporter)
import Test.Spec.Runner.Node (runSpecAndExitProcess)
import Test.TestUtils (timeout)
import Zelkova.Signal (constant, dropRepeats, filter, filterMap, flatten, foldp, get, mergeMany, runSignal, sampleOn, (<~), (~), (~>))
import Zelkova.Signal.Aff (mapAff)
import Zelkova.Signal.Channel (subscribe, send, channel)
import Zelkova.Signal.Effect (foldEffect, mapEffect)
import Zelkova.Signal.Time (since, delay, every, debounce)

next :: forall a. a -> Effect a
next val = pure val
...
main =
  runSpecAndExitProcess [ consoleReporter ]
    $ sequential do
        describe "Zelkov.Signal" do
          describe "constant" do
            itOnly "yields once"
              $ do
                  expect 1 (constant "lol") [ "lol" ]
                  results <- runSignal $ (constant "lol")
                  log $ show results
          it "run tests" do
            ...
            test "debounce yields only the most recent value in a series shorter than the interval" do
              ...
              let
                sig = debounce 10.0 $ subscribe chan
              ...
              makeAff
                $ \resolve -> do
                    ...
                    if (val == "example") then
                      ...
                      resolve $ Left $ error ("Expected get sig to return \"example\" but got " <> val)
                    pure nonCanceler
